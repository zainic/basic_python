...
for (x, y, w, h) in L_eye:
    ...
    for (a, b, c, d) in R_eye:
        ...
        Nose_che = gray[U:D, L:R]
        pixel_values = Nose_check.reshape((-1))
        cv2.imshow('Nose_Original', Nose_che)
        ...
        cv2.imshow('Nose_K-Means', Nose_check2)
        # Global Threshold
        ret, Nose = cv2.threshold(Nose_check2, 127, 255, cv2.THRESH_BINARY)
        # View Nose Region
        cv2.imshow('Nose_Threshold', Nose)
        ...
        print(ACC)
        if ACC >= 0.99:
            cv2.rectangle(img, (L2, U2), (R2, D2), (0, 255, 0), 3)
            print('MASK IS DETECTED!!')
        else:
            cv2.rectangle(img, (L2, U2), (R2, D2), (0, 0, 255), 3)
